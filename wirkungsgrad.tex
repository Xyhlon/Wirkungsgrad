...
\begin{itemize}
	\item Messung der Dunkelkennlinie: An der einzelnen, abgedeckten Solarzelle wird mit
	      ...
	      I(U)-Verlauf ist mit der gegebenen Formel zu fitten und die Parameter der
	      Diode sind zu bestimmen.
...
%todo wird mit richtiger zitierung hoffentlich kleiner
\begin{table}[H]
...
\dq Einstellungen anzeigen\dq \ verbunden wurde, muss sichergestellt werden, dass die
...
%todo in tab steht p_w und nicht p_h
\begin{table}[H]
...
analogen Messgerät abgelesen, was gemeinsam mit den entsprechenden Größen
 in folgender Aufzählung in \Cref{enum:werte_solarzelle2}
...
\autoref{fig:mess_hellkennlinie_lampe} bis
...
\begin{figure}[H]
	...
	\caption[Zeitliche Verläufe von den Drücken
	und Temperaturen, während dem Betrieb der Wärmepumpe]{Zeitliche Verläufe von den Drücken
...
%todo kommando das q zusammen bleibt 
...
\begin{figure}[H]
	...
	\caption[Verlauf der
	Leistungszahl]{Verlauf der
		Leistungszahl $\epsilon$ einer Wärmepumpe, während diese durch ihren Betrieb
...
Maximalen $\epsilon_\text{max}$ gesetzt wird, kann der Gütefaktor $\eta$ bestimmt
werden, wie in \autoref{eq:guetefaktor} sichtbar. Dabei wurde $\epsilon_\text{max}$
...
\begin{figure}[H]
	...
	\caption[Dynamischer Gütegradverlauf $\eta$ der
	Wärmepumpe]{Dynamischer Gütegradverlauf $\eta$ der
		Wärmepumpe. Der Gütegrad $\eta$ wird bezüglich der
		...
		$\epsilon_\text{max}$ ist auch angeführt.
...
Um die Kenndaten zu erzeugen sind folgende Statistiken auf die Datensätze angewandt worden.
Damit der Kurzschlussstrom $I_k$ ermittelt werden kann ist der Messwert bei der
Spannung von $\SI{0}{\volt}$, für die Leerlaufspannung $U_L$
der Messwert mit keinem Strom $\SI{0}{\ampere}$ genommen worden. Für den Betriebspunkt der
maximalen Leistung ist zunächst die Leistung mit $P=UI$ berechnet und
von diesen Werten das Maximum genommen worden, um $P_\text{MPP}$ zu
bestimmen, und die Spannung $U_\text{MPP}$ und Strom $I_\text{MPP}$, die diese
Leistung produzierten, vermerkt worden. Dieses Verfahren ist für die verschiedenen
Beschaltungen und Belichtungsflächen durchgeführt worden.
...
\begin{figure}[H]
	...
	\caption[Kennlinien Serienschaltung mit teils bedeckten Solarzellen]{Untersuchung der
		...
		bei einer Serienschaltung mit Teilbedeckung der Solarzellen. Folgende Größen wurden errechnet und, um
		Effizienz für dieser Konfiguration besser vergleichen zu können, auch angeführt:            \\
		$I_k \dots$ Kurzschlussstrom         \\
		$U_L \dots$ Leerlaufspannung         \\
		$P_\text{MPP} \dots$ Leistung am MPP \\
...
Anteile werden vernachlässigt, was bewirkt dass die gefittet Funktion höher sein wird als
...
Tail zu großen Einfluss auf den Fit hatte. Die Kurve hat viel zu spät zum Steigen
...
\begin{figure}[H]
	...
	\caption[Dunkelkennlinie einer Solarzelle]{Untersuchung der Strom-Spannungs Kennlinie
		...
		Folgende Größen wurden errechnet und, um
		Effizienz für dieser Konfiguration besser vergleichen zu können, auch angeführt: \\
		$P_\text{MPP} \dots$ Leistung am MPP         \\
		$I_\text{MPP} \dots$ Strom am MPP            \\
		$U_\text{MPP} \dots$ Spannung am MPP         \\
...
Für die Hellkennlinien musst der Tail nicht weniger gewichtet werden, da die Fits
auch so zu einem sinnvollen Ergebnis konvergiert sind.
...
\begin{figure}[H]
	...
	\caption[Kennlinie durch Halogen-Lampen 1.te Belichtung]{Untersuchung der
		...
		mittels einer Halogenlampe. Folgende Größen wurden errechnet und, um
		Effizienz für dieser Konfiguration besser vergleichen zu können, auch angeführt: \\
		$P_\text{MPP} \dots$ Leistung am MPP                                 \\
		$I_\text{MPP} \dots$ Strom am MPP                                    \\
		$U_\text{MPP} \dots$ Spannung am MPP                                 \\
...
\begin{figure}[H]
	...
	\caption[Kennlinie durch LED-Lampen Belichtung]{Untersuchung der Strom-Spannungs
		...
		bläulichen LED-Lampe. Folgende Größen wurden errechnet und, um
		Effizienz für dieser Konfiguration besser vergleichen zu können, auch angeführt: \\
		$P_\text{MPP} \dots$ Leistung am MPP                             \\
		$I_\text{MPP} \dots$ Strom am MPP                                \\
		$U_\text{MPP} \dots$ Spannung am MPP                             \\
...
Beim Betrachten der Grafik in \autoref{fig:druckTemperaturVerlauf} wird klar ersichtlich,
dass es besser wäre die Flüssigkeit
immer konstant umzurühren, da sonst, immer wenn umgerührt wird, Spikes in den Temperatur 
und Druckverläufen entstehen, die vermeiden werden sollten.
...
einer erneuten Durchführung des Versuchs sollte daher die Flüssigkeit, nicht nur
wegen der Spikes sondern auch gegen die Eisbildung deutlich
...
%todo bsp auch bringen
Bei einer erneuten Durchführung wäre es wichtig, die Lampe einige Zeit auf 
die Solarzelle scheinen zu lassen, damit sich eine konstante Betriebstemperatur 
...
bei einem niedrigeren Strom liefert, als
...
%?
Bei der Dunkelkennlinie entsprechen die meisten Parameter null, wie $I_{ph}$, was auch mit der
...
Bei der bläulichen LED Lampe wird Energie schlechter von der Solarzellen aufgenommen. 
Deshalb wird trotz der hohen Bestrahlungsstärke kaum etwas konvertiert, da
